...
describe 'resource_plex_home_theater_app::windows::2012r2' do
  ...
  context 'the default action (:install)' do
    ...
    before(:each) do
      body = 'https://downloads.plex.tv/plex-home-theater/4/' \
    ...
    shared_examples_for 'any attribute set' do
      it 'installs the package file' do
        expect(chef_run).to install_windows_package('Plex Home Theater')
          .with(source: "#{Chef::Config[:file_cache_path]}/PlexHomeTheater.4" \
                        '-windows-x86.exe',
                installer_type: :nsis)
      end
    end
    ...
    shared_examples_for 'app not already installed' do
      it 'downloads the package file' do
        ...
        ).with(source: source || 'http://example.com/plex.exe')
      end
    end
